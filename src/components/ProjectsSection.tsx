...
const ProjectCard = ({
  ...
  return <Card className="project-card hover:scale-[1.02] transition-transform bg-blue-800">
      <div className="relative">
        <div className={`absolute top-0 left-0 h-full w-2 ${isEven ? 'bg-blue-600' : 'bg-blue-500'}`}></div>
        <div className="p-6 pl-8">
          <h3 className="text-xl font-bold mb-3 text-white">{title}</h3>
          <p className="text-blue-100 text-left">{description}</p>
        </div>
      </div>
    </Card>;
};
const ProjectsSection = () => {
  ...
  return <section id="projects" className="py-20 bg-gray-100">
      <div className="container mx-auto px-4">
        <h2 className="section-heading">Our Projects</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-6xl mx-auto">
          {projects.map((project, index) => <ProjectCard key={index} title={project.title} description={project.description} index={index} />)}
        </div>
